...
-(instancetype)initWithBotTTTSymbol:(NSString *)symbol botStartsTheGame:(BOOL)botStartsTheGame{
    ...
    if (self) {
        ...
        NSArray *initialBoard = @[@1, @2, @3, @4, @5, @6, @7, @8, @9];
...
-(NSDictionary<NSString *, id> *)nextPossibleBoardsWithBoard:(NSArray *)parentBoard depthLevel:(NSUInteger)depthLevel {
    ...
    NSArray<NSNumber *> *emptyPositions = [parentBoard filteredArrayUsingPredicate:[NSPredicate predicateWithFormat:@"self isKindOfClass: %@", [NSNumber class]]];
    NSMutableArray<NSDictionary *> *nextPossibleBoards = [[NSMutableArray alloc] init];
//    float score = 0.f;
    float score = 0.f;
    for (NSNumber *position in emptyPositions) {
        NSMutableDictionary *eachMove = [NSMutableDictionary dictionary];
        NSArray *board = [self boardWithBoard:parentBoard
                               positionToMark:[position unsignedIntegerValue]-1
...
//        if (self.botStartsTheGame && depthLevel % 2 == 0) {
//            //Bot's Turn.
//            eachMove[kScoreKey] = [self maxScoreFromFirstGenerationChildren:possibleBoards];
//        } else if (self.botStartsTheGame && depthLevel % 2 == 1) {
//            //Player's Turn.
//            eachMove[kScoreKey] = [self minScoreFromFirstGenerationChildren:possibleBoards];
//        } else if (!self.botStartsTheGame && depthLevel % 2 == 1) {
//            //Bot's Turn.
//            eachMove[kScoreKey] = [self maxScoreFromFirstGenerationChildren:possibleBoards];
//        } else if (!self.botStartsTheGame && depthLevel % 2 == 0){
//            //Player's turn
//            eachMove[kScoreKey] = [self minScoreFromFirstGenerationChildren:possibleBoards];
//        }
...
-(NSMutableArray *)boardWithBoard:(NSArray *)parentBoard positionToMark:(const NSUInteger)positionToMark depthLevel:(NSUInteger)depthLevel{
    NSMutableArray *board = [parentBoard mutableCopy];
    if (depthLevel % 2 == 1) {
...
-(BOOL)gameIsCompleteWithBoard:(NSArray *)board {
    NSArray<NSNumber *> *emptyPositions = [board filteredArrayUsingPredicate:[NSPredicate predicateWithFormat:@"self isKindOfClass: %@", [NSNumber class]]];
...
-(NSArray<NSNumber *> *)winningIndicesWithBoard:(NSArray *)board {
    if (board[0] == board[4] && board[4] == board[8]) {
        NSArray<NSNumber *> *winningIndices = @[@0,@4,@8];
        return winningIndices;
    }
    self.numberOfComparisons++;
...
// Returns @"X", @"O", or nil
-(NSString *)winnerWithCompleteBoard:(NSArray *)board {
    NSArray<NSNumber *> *emptyPositions = [board filteredArrayUsingPredicate:[NSPredicate predicateWithFormat:@"self isKindOfClass: %@", [NSNumber class]]];
...
-(NSNumber *)scoreWithBoard:(NSArray *)board {
    // Return 0 if it's a tie, 1 if we won, and -1 if we lost
    // Who is -we- here? The bot? The bot makes sense
    float score = 0.f;
